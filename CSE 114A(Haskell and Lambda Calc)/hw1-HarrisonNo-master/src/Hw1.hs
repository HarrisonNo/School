...
digitsOfInt n
    | n < 1     = []
    | otherwise = (digitsOfInt (quot n 10)) ++ [mod n 10] --Appends mod n 10 to list so that it isn't in reverse order
...
singleDigitRoot :: Int -> Int --helper function that only finds the next digitalroot and not the final one
singleDigitRoot n
    | n < 10    = n
    | otherwise = (mod n 10) + singleDigitRoot (quot n 10)
...
additivePersistence n
    | n < 10    = 0
    | otherwise = 1 + additivePersistence (singleDigitRoot n)
...
digitalRoot n
    | n < 10    = n
    | otherwise = digitalRoot (singleDigitRoot n)
...
palindrome w = w==listReverse w --Inherently returns true or false based on if this statement is true/false