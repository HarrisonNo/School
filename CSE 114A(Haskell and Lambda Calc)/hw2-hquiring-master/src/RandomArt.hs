...
eval x y e = case e of
                VarX             -> x
                VarY             -> y
                (Sine i)         -> sin(pi * eval x y i)
                (Cosine i)       -> cos(pi * eval x y i)
                (Average i1 i2)  -> (eval x y i1)/(eval x y i2)
                (Times i1 i2)    -> (eval x y i1) * (eval x y i2)
                (Thresh a b c d) -> if ((eval x y a)<(eval x y b)) then (eval x y c) else (eval x y d)
...
build 0
  ...
  where
    r         = rand 10
build d
  | (mod d 14) < 0    = Times VarX VarY      
  | (mod d 14) < 2    = VarX
  | (mod d 14) < 4    = VarY
  | (mod d 14) < 6    = Sine (build(d-1))
  | (mod d 14) < 8    = Cosine (build(d-1))
  | (mod d 14) < 10   = Average (build(d-2)) (build(d-1))
  | (mod d 14) < 12   = Times (build(d-1)) (build(d-3)) 
  | otherwise       = Thresh (build 1) (build 3) (build (d-1)) (build (d-2))
