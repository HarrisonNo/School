...
eval _ (EBool a)             = (VBool a)
eval _ ENil                  = VNil 
eval env (EVar a)            = (lookupId a env)
eval env (EBin operator a b) = (evalOp operator (eval env a) (eval env b))
eval env (EIf a b c)         = if ((eval env a) == (VBool True)) then (eval env b) else (eval env c)
eval env (ELet id a b)       = (eval newenv b)
                                  where
                                    f      = eval newenv a
                                    newenv = [(id,f)] ++ env
eval env (EApp a b)          = case (eval env a) of
  (VClos nenv id expr)       -> (eval ([(id, (eval env b))] ++ nenv) expr)
  (VPrim f)                  -> (f (eval env b))
  otherwise                  -> (VErr (throw (Error "type error")))
eval env (ELam id a)         = (VClos env id a)
eval _ _                     = (VErr (throw (Error "type error")))
--------------------------------------------------------------------------------
...
evalOp Eq a b                           = (VBool (a == b))
evalOp Ne a b                           = (VBool (a /= b))
evalOp Lt (VInt a) (VInt b)             = (VBool (a < b))
evalOp Le (VInt a) (VInt b)             = (VBool (a <= b))
evalOp And (VBool a) (VBool b)          = if ((VBool a) == (VBool True)) then (VBool b) else (VBool False)
evalOp Or (VBool a) (VBool b)           = if ((VBool a) == (VBool True)) then (VBool True) else (VBool b)
evalOp Cons (VPair a VNil) (VPair x xs) = (VPair a (VPair x xs))
evalOp Cons (VPair a b) (VPair x xs)    = (VPair a (evalOp Cons (b) (VPair x xs)))
evalOp Cons a b                         = (VPair a b)
evalOp _ _ _                            = (VErr (throw (Error "type error")))
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
lookupId _ []        = VErr (throw (Error "unbound variable"))
lookupId a ((x,y):xs)  = if (a==x)
                        then (y)
                        else (lookupId a xs)

headfunc :: Value -> Value
headfunc (VPair a _) = a
headfunc _           = VErr (throw (Error "empty list"))


tailfunc :: Value -> Value
tailfunc (VPair _ b) = b
tailfunc _           = VErr (throw (Error "empty list"))
